...
export function formatChaiMenu(items) {
  ...
  const filterItem = items.filter((item) => typeof (item.name) === "string" && item.name.trim() !== "" && item.price > 0);

  const menuItems = filterItem.map((item) => `${item.name.toUpperCase()} - Rs.${item.price}`).join(" | ");
